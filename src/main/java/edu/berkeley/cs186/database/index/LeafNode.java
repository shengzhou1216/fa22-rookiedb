...
class LeafNode extends BPlusNode {
    ...
    public void remove(DataBox key) {
        ...
        rids.remove(n);
        keys.remove(n);
        // serialize this leaf to its page.
        sync();
        return;
    }
    ...
    public byte[] toBytes() {
        ...
        // (3,1) is RecordId(pageNum,entryNum)
        ...
        buf.put((byte) 1); // a
        buf.putLong(rightSibling.orElse(-1L)); // b
        buf.putInt(keys.size()); // c
        for (int i = 0; i < keys.size(); ++i) { // d0,d1,d2,...
